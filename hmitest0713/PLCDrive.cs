...
namespace hmitest0713
{
    class PLCDrive
    {
        ...
        public TCPClient Client { get { return _client; } set {; } }

        public PLCDrive(TCPClient client)
        {
            _client = client;
        }
        ...
        public float[] ReadFloat(string address, ushort length)
        {
            ...
            byte[] command = MelsecHelper.BuildAsciiReadMcCoreCommand(mcAddressData, false);
        ...
        {
            bool success = this._client.socketSend(_client.keySocket, v);
            if (success)
            {
                return this._client.socketReceive();
            }
            return null;
        ...
        public class McAddressData : DeviceAddressDataBase
        {
            ...
            {
                McAddressData addressData = McAddressData.ParseKeyenceFrom(address, length);
                if (addressData.IsSuccess)
                {
                    this.AddressStart = addressData.AddressStart;
                    this.Length = addressData.Length;
                    this.McDataType = addressData.McDataType;
                }
            }
            ...
            public static McAddressData ParseKeyenceFrom(string address, ushort length)
            {
                ...
                try
                {
                    switch (address[0])
                    {
                        ...
                        default: break;//throw new Exception(StringResources.Language.NotSupportedDataType);
                    }
                }
                catch (Exception ex)
                {
